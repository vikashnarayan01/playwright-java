...
public class AppTest {
	...
	public void setUp() {
		...
		apiRequestContext = playwright.request().newContext(
				new APIRequest.NewContextOptions().setBaseURL("http://localhost:8888").setExtraHTTPHeaders(headers));
	}
	...
	public void CRUDCallVerification() {
		...
		//
		// String response = apiRequestContext.post("/comments",
		// RequestOptions.create().setData(payLoadComment)).text();
		...
		String responsePatch = apiRequestContext.delete("/comments/6", RequestOptions.create().setData(payLoadComment))
				.text();
